'use strict';
// 平台管理模块的配置
angular.module('lotteryWeixinModule', ['userWeixinModule']).config(function($stateProvider) {

	$stateProvider.state('lottery', {
		url : "/lottery?id",
		controller : 'lotteryMainCtrl',
		templateUrl : "weixin/views/lottery/lottery.html"
	});

	// 服务配置
}).service("lotteryRestService", function($resource, commonService) {
	
	return $resource("lottery/:id", {id : "@id"}, commonService.getDefaultRestSetting());
	
}).controller("lotteryMainCtrl", function($scope, userRestService) {
...
}).directive('pzLottery', function($stateParams, lotteryRestService, commonService) {
	return {
		restrict : 'A',
			...
			lotteryRestService.get({id: $stateParams.id}).$promise.then(function(result){
				...
				var rotateFn = function (item, txt){
					...
					$('#wheelcanvas').rotate({
						...
						duration:8000,
						callback:function (){
							lotteryRestService.save({lotteryId: $stateParams.id, prize: txt}).$promise.then(function(){
								if(txt.indexOf("谢谢参与") != -1){
									commonService.showInfo("谢谢您的参与,祝您下次好运");
								}else{
									commonService.showInfo("恭喜您,您抽中了"+txt);
								}
				...
				$('.pointer').click(function (){
					...
					lotteryRestService.create({lotteryId: $stateParams.id}).$promise.then(function(result){
						turnplate.bRotate = !turnplate.bRotate;
						//获取随机数(奖品个数范围内)
